...
headers = {
    ...
    'Cookie': 'JSESSIONID=C6707C1BC0977C1154A142BF351E2F1D; LGMOID=20151221133025-13872EFC4C9A30161B385C5123184A1F; user_trace_token=20151221133026-f0b87a3c-a7a3-11e5-86e4-525400f775ce; PRE_UTM=AD__baidu_pinzhuan; PRE_HOST=bzclk.baidu.com; PRE_SITE=http%3A%2F%2Fbzclk.baidu.com%2Fadrc.php%3Ft%3D0fKL00c00fA-iwf0g0Fw0FNkUsKsWMIy000007KSy1s00000AkDWg9.THL0oUhY0A3qmh7GuZR0T1dbrADdPW9-rH0snjRzrj0v0ZRqrDDdrj6LrHnsfW-7f177rjfYrjNAfYm3wbuKnHcYwRc0mHdL5iuVmv-b5Hc1P1f1PHnsPWThTZFEuA-b5HDvFhqzpHYkFMPdmhqzpHYhTZFG5Hc0uHdCIZwsrBtEILILQhk9uvqdQhPEUitOIgwVgLPEIgFWuHdKw7qxmh7GuZNxTA-8Xh9dmy3hIgwVgvd-uA-dUHd1uyYhIgwVgvP9UgK9pyI85NP7HfKWThnqnHb1PH0%26tpl%3Dtpl_10085_12986_1%26l%3D1012629072%26ie%3DUTF-8%26f%3D8%26tn%3Dbaidu%26wd%3D%25E6%258B%2589%25E5%258B%25BE; PRE_LAND=http%3A%2F%2Fwww.lagou.com%2F%3Futm_source%3DAD__baidu_pinzhuan%26utm_medium%3Dsem%26utm_campaign%3DSEM; LGUID=20151221133026-f0b8818a-a7a3-11e5-86e4-525400f775ce; index_location_city=%E5%85%A8%E5%9B%BD; SEARCH_ID=eedf2b3b199c484c97a230ae5d898213; Hm_lvt_4233e74dff0ae5bd0a3d81c6ccf756e6=1450675826; Hm_lpvt_4233e74dff0ae5bd0a3d81c6ccf756e6=1450675832; _ga=GA1.2.679801410.1450675826; LGSID=20151221133026-f0b87e3e-a7a3-11e5-86e4-525400f775ce; LGRID=20151221133032-f40ba594-a7a3-11e5-8976-5254005c3644',
...
if __name__ == '__main__':
    companyName = positions['content']['result'][0]['companyName']
    positionId = positions['content']['result'][0]['positionId']
    city = positions['content']['result'][0]['city']
    companyId = positions['content']['result'][0]['companyId']
    education = positions['content']['result'][0]['education']
    companySize = positions['content']['result'][0]['companySize']
    financeStage = positions['content']['result'][0]['financeStage']
    financeStage = financeStage[0:financeStage.find('(')]
    print(financeStage)
    industryField = positions['content']['result'][0]['industryField']
    positionName = positions['content']['result'][0]['positionName']
    positionFirstType = positions['content']['result'][0]['positionFirstType']
    positionType = positions['content']['result'][0]['positionType']
    format = '0123456789'
    for c in companySize:
        if not c in format:
            companySize = companySize.replace(c, '')
    print(companySize)
    salary = positions['content']['result'][0]['salary']
    salary = (int(salary[0:salary.find('k')]) + int(salary[salary.find('-')+1:len(salary)-1])) / 2
    workYear = positions['content']['result'][0]['workYear']
    workYear = workYear[0:workYear.find('-')]


    print(salary)
    print(workYear)
    with open('java.csv', 'wb') as csvfile:
